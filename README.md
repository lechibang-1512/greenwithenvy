...
GreenWithEnvy is a CLI tool that provides an easy way to switch between GPU modes on Nvidia Optimus systems (i.e laptops with hybrid Intel + Nvidia or AMD + Nvidia graphics configurations) under Linux.
...
```
usage: greenwithenvy.py [-h] [-v] [-q] [-s MODE] [--dm DISPLAY_MANAGER] [--force-comp] [--coolbits [VALUE]] [--rtd3 [VALUE]] [--reset-sddm] [--reset] [--verbose]
...
options:
  ...
  --reset               Revert changes made by GreenWithEnvy
  --cache-create        Create cache used by GreenWithEnvy; only works in hybrid mode
  --cache-delete        Delete cache created by GreenWithEnvy
  --cache-query         Show cache created by GreenWithEnvy
...
Revert all changes made by GreenWithEnvy:

```
sudo greenwithenvy --reset
```
...
Create cache used by GreenWithEnvy; only works in hybrid mode

```
sudo greenwithenvy --cache-create
```
...
Delete cache created by GreenWithEnvy

```
sudo greenwithenvy --cache-delete
```

Show cache created by GreenWithEnvy

```
sudo greenwithenvy --cache-query
```


## ⬇️ Getting GreenWithEnvy

### Arch Linux ([AUR](https://aur.archlinux.org/packages/greenwithenvy))

1. `yay -S greenwithenvy`
2. Run `sudo greenwithenvy -s <MODE>` to switch graphics modes
...
3. Run `sudo greenwithenvy -s <MODE>` to switch graphics modes
...
3. Overlay the package with `rpm-ostree install python-greenwithenvy`.
4. Reboot to apply the overlay.
5. Use GreenWithEnvy with `sudo greenwithenvy -s <MODE>`
...
- Now you can run `sudo greenwithenvy -s <MODE>` from any directory to switch graphics modes.
...
**Make sure to have GreenWithEnvy installed globally!**
...
If you're running into this situation you can use the `--use-nvidia-current` flag to make GreenWithEnvy use the correct module name.

### Wayland session is missing on Gnome 43+

GDM now requires `NVreg_PreserveVideoMemoryAllocations` kernel parameter which breaks sleep in nvidia and hybrid mode, as well as rtd3 in hybrid mode, so GreenWithEnvy disables it, if you need a Wayland session follow the instructions below
...
If this ever happens please run `sudo greenwithenvy --reset-sddm`.

### Files to remove if uninstalling `greenwithenvy`
The below files are created by `greenwithenvy`, and you may want to remove them manually if they are not removed automatically to avoid any incorrect system behaviour.
* `/var/cache/greenwithenvy`
